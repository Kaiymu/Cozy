    ...
    public static class Ocean
    {
        ...
        public static void BakeAndPackWaveData()
        {
            ...
            for (int w = 0; w<4; w++)
            {
                ...
                groupSpeed[w] = Mathf.Min(1f, intensity[w]*intensity[w])*0.5f*Mathf.Sqrt(1.5613f*wavelength[w]);
        ...
        static public Vector3 OceanDeformation(float time, Vector3 undeformedPosition, out Vector3 breakingVelocity)
        {
            ...
            for (int w = 0; w<4; w++)
            {
                ...
                float deltav = 0.085f*wavelength[w]*(0.2f+Mathf.Cos(Mathf.PI*Mathf.Pow(relativePhase, 1f+clampedAmplitude*0.5f)));
                ...
                if (unclampedAmplitude > 1f && deltah > 0f && isFrontOfTheGroup)
                {
                    ...
                    if (Mathf.Sin(centerGroupPhase) > 0f)
                    {
                        ...
                        breakingVelocity -= groupSpeed[w]*(unclampedAmplitude-1f)*breakSpeedFactor*directionVector[w];
                        ...
                        breakingVelocity.y += (unclampedAmplitude-1f)*wavelength[w]*0.01f;
                    }
                }
        ...
        static public float GetHeight(float time, Vector3 position, ref Vector3 previousUndeformedPosition, out Vector3 deformation, bool recomputeOcean = true)
        {
            ...
            //first we need to calculate the ocean deformation on the current undeformed water position (whih is still the previous one) so to compare to position
            deformation =  OceanDeformation(time, previousUndeformedPosition, out _);
        ...
        static public Vector3 GetNormal(float time, Vector3 undeformedPosition, Vector3 alreadyComputedDeformation, float precision = 0.1f)
        {
            ...
            Vector3 deformedVector1 = undeformedPosition + alreadyComputedDeformation - undeformedForwardPoint - OceanDeformation(time, undeformedForwardPoint, out _);
            Vector3 deformedVector2 = undeformedPosition + alreadyComputedDeformation - undeformedLeftPoint    - OceanDeformation(time, undeformedLeftPoint, out _);
        ...
        static public Vector3 GetVelocity(float time, Vector3 undeformedPosition, Vector3 alreadyComputedDeformation, out Vector3 breakingTorque, float depth = 0f, float dt = 0.1f)
        {
            ...
            Vector3 furtureDeformedPosition = undeformedPosition + OceanDeformation(time + dt, undeformedPosition, out Vector3 breakVelocity);
