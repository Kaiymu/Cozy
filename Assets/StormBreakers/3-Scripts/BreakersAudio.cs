            ...
            for (int w = 0; w < 4; w++)
            {
                ...
                for (int e = 0; e < 9; e++)
                {
                    ...
                    if (Ocean.areBreakers)
                    {
                        ...
                        float phase = Vector3.Dot(emitterPosition, Ocean.directionVector[w])*Ocean.wavenumber[w] + Time.time*Ocean.pulsation[w] + unphasing[w, e] + amplitudeVariation*amplitude[w, e];

                        // calculating the compression
                        float compression = amplitudeVariation*amplitude[w, e]*Mathf.Cos(phase -1.2f);

                        // when the compression exceed the trigger, then the audio parameters can be generated
                        if (amplitude[w, e] > 1f && compression > 0f)
                        {
                            // adding the wave advance to the position to make the sound closer to the actual wave  
                            emitterPosition += (-0.1f*amplitude[w, e]  - 0.12f*Mathf.Sin(phase))*Ocean.directionVector[w]*Ocean.wavelength[w] + 0.085f*Vector3.up*amplitude[w, e]*Ocean.wavelength[w];
