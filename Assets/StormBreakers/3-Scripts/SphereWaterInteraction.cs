...
namespace StormBreakers
{
    public class SphereWaterInteraction : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            waterHeight = Ocean.GetHeight(Time.time, transform.position, ref undeformedPosition, out Vector3 deformation);
            ...
            if (depth > 0f)
            {
                waterNormal = Ocean.GetNormal(Time.time, undeformedPosition, deformation, radius);
                waterVelocity = Ocean.GetVelocity(Time.time, undeformedPosition, deformation, out _, depth);
            }
